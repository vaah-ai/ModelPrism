...
>
> **Impact from M1-T8:** Dashboard frontend scaffolded at `dashboard-gpu-server/` with composable `useWebSocketMetrics.ts` that expects WebSocket endpoint at `ws://<backendUrl>/ws/dashboard`. Supported message types: `metrics`, `agent_status` (for status transitions), `agent_update` (for field-level updates), `replay_batch` (for replay on reconnect). Messages must carry `type` discriminator and `agent_id`. Frontend batches messages for 500ms then dispatches to Pinia stores. Auto-reconnect with 3s base delay, exponential backoff to 30s max. The composable sends `{"type": "ping"}` keepalive — backend should respond with `{"type": "pong"}`.
...
> **Impact from M1-T6:** Metrics are now persisted to PostgreSQL via `MetricIngestionService` in `app/services/metric_service.py`. The WebSocket handler in `app/ws/agent_ws.py` has been extended: `_handle_metrics()` now calls `metric_service.enqueue()` to buffer rows for batch DB writes, and `_handle_vllm_metrics()` calls `metric_service.enqueue_vllm()`. The Redis pub/sub pattern (`metrics:{id}`, `vllm_metrics:{id}`) remains unchanged, so the dashboard WS broadcast for this task can subscribe to the same Redis channels. Latest metrics are stored at `agent:{id}:latest_metrics` (same key as before). The downsampling background worker (`app/services/downsampler.py`) runs as an asyncio task in the FastAPI lifespan. For the dashboard broadcast, subscribe to `metrics:{id}` and `vllm_metrics:{id}` Redis channels — these channels are still published by the WS handler on every 2s metric push.
